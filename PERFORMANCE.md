...
deque was based on impl7. As verified by the above test results, deque performs
...
Similarly to other deques tested, slice, when used as a FIFO queue,
performs marginally better than deque with very small data sets,
but fall behind quite dramatically with large data sets, performance and
memory wise. Unless the only worry is performance with very small data sets (<=10),
I don't recommend using slice as a simple FIFO queue.
